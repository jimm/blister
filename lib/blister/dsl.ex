defmodule Blister.DSL do
  ...
  defp parse_songs([s | t], inputs, outputs, songs) do
    with {:ok, patch_pids} <- parse_patches(Map.get(s, :patches, []), inputs, outputs, []) do
  ...
  defp parse_patches([p | t], inputs, outputs, patches) do
    with {:ok, conns} <-
           get_any(p, [:connections, :conns], [])
           |> parse_connections(inputs, outputs, []) do
      start_msgs =
        case get_any(p, [:start_messages, :start_msgs, :start]) do
          nil -> []
          ms when is_list(ms) -> ms |> Enum.map(&normalize_message/1)
        end

      stop_msgs =
        case get_any(p, [:stop_messages, :stop_msgs, :stop]) do
          nil -> []
          ms when is_list(ms) -> ms |> Enum.map(&normalize_message/1)
        end
  ...
  defp parse_connection_io({in_sym, out_sym, out_ch}, inputs, outputs)
       when is_atom(in_sym) and is_atom(out_sym) and is_integer(out_ch) do
