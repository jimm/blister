defmodule Blister.Cursor do
  ...
  def prev_song_list(cursor, pack, which_song, which_patch) do
    ...
    if new_song_list_index < 0 do
      ...
      case which_song do
        :first ->
          %{cursor | song_list_index: new_song_list_index, song_list: song_list, song_index: -1}
          |> next_song(pack)
  ...
  def mark(cursor) do
    %{
      cursor
      | song_list_name: if(cursor.song_list, do: cursor.song_list.name),
  ...
  def restore(cursor, pack) do
    ...
    song =
      find_nearest_match(song_list.songs, cursor.song_name) || first_index_of(song_list.songs)
  ...
  def find_nearest_match(list, str) do
    ...
    distances =
      list
      |> Enum.map(
        &damerau_levenshtein(str |> to_char_list, &1.name |> String.downcase() |> to_char_list)
      )
      ...
      |> Enum.reduce({nil, thisrow}, fn x, {twoago, oneago} ->
        ...
        thisrow =
          ...
          |> Enum.reduce(thisrow, fn y, trow ->
            ...
            if x > 0 && y > 0 && Enum.at(seq1, x) == Enum.at(seq2, y - 1) &&
                 Enum.at(seq1, x - 1) == Enum.at(seq2, y) && Enum.at(seq1, x) != Enum.at(seq2, y) do
