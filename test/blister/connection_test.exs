defmodule Blister.ConnectionTest do
  ...
  setup do
    conn = %Connection{
      ...
      filter: fn _, {b0, b1, b2} = msg ->
        if P.note?(msg), do: {b0, b1, max(0, b2 - 1)}, else: msg
      end,
      zone: 64..75,
      xpose: 12
    }
  ...
  test "passes through non-channel messages", context do
    msgs =
      Connection.process(context[:conn], [{C.tune_request(), 0, 0}])
      |> Enum.to_list()
  ...
  test "passes through correct channel and range and munges", context do
    ...
    msgs =
      Connection.process(context[:conn], [{C.note_on() + 4, 65, 118}])
      |> Enum.to_list()
  ...
  test "filters out different channels", context do
    msgs =
      Connection.process(context[:conn], [{C.note_on(), 65, 118}])
      |> Enum.to_list()
  ...
  test "filters out notes outside zone", context do
    msgs =
      Connection.process(context[:conn], [{C.note_on(), 13, 118}])
      |> Enum.to_list()
